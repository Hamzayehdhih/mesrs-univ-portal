...
('Bourse d\'Excellence MESRS', 'merit', 150000, 12, 'Moyenne générale > 16/20', '2024-12-31', (SELECT id FROM academic_years WHERE is_current = true), 50, 'Bourse pour les étudiants méritants', true),
...
((SELECT id FROM students WHERE student_number = 'ST2024002'), (SELECT id FROM scholarships WHERE name = 'Bourse d''Excellence MESRS'), 'approved', '2024-10-01', 'Je suis un étudiant passionné par les mathématiques...', 18.5),
...
-- Add some historical enrollment data for trends
INSERT INTO public.enrollments (student_id, formation_id, academic_year_id, status, enrollment_date, notes) VALUES
-- Previous year data
((SELECT id FROM students WHERE student_number = 'ST2024002'), (SELECT id FROM formations WHERE code = 'MATH-L3'), (SELECT id FROM academic_years WHERE is_current = false), 'completed', '2023-09-01', 'Année précédente terminée');
