-- Create simple sample data without auth dependencies

-- First, let's ensure we have some basic data that doesn't require user authentication
...
SELECT 'UNAM', 'Université de Nouakchott Al Aasriya', 'Nouakchott, Mauritanie', '+222 45 25 13 82', 'contact@una.mr', 'http://www.una.mr', 'Dr. Ahmed Ould Mohamed', 15000, true
WHERE NOT EXISTS (SELECT 1 FROM public.universities WHERE code = 'UNAM');

INSERT INTO public.universities (code, name, address, phone, email, website, rector_name, student_count, is_active) 
SELECT 'UIZ', 'Université Islamique de Zouerate', 'Zouerate, Mauritanie', '+222 44 59 12 34', 'contact@uiz.mr', 'http://www.uiz.mr', 'Dr. Fatima Mint Ahmed', 3500, true
WHERE NOT EXISTS (SELECT 1 FROM public.universities WHERE code = 'UIZ');

INSERT INTO public.universities (code, name, address, phone, email, website, rector_name, student_count, is_active) 
SELECT 'UNA', 'Université de Nouadhibou', 'Nouadhibou, Mauritanie', '+222 45 74 82 91', 'contact@unouadhibou.mr', 'http://www.unouadhibou.mr', 'Prof. Mohamed Ould Saleh', 2800, true
WHERE NOT EXISTS (SELECT 1 FROM public.universities WHERE code = 'UNA');

INSERT INTO public.universities (code, name, address, phone, email, website, rector_name, student_count, is_active) 
SELECT 'USTM', 'Université des Sciences et Technologies de Mauritanie', 'Nouakchott, Mauritanie', '+222 45 25 67 89', 'contact@ustm.mr', 'http://www.ustm.mr', 'Dr. Aminata Mint Mohamed', 4200, true
...
INSERT INTO public.formations (code, name, description, university_id, credits, duration_years, capacity, is_active) 
SELECT 'PHYS-L3', 'Licence en Physique', 'Formation en physique et sciences appliquées', u.id, 180, 3, 60, true
FROM public.universities u
WHERE u.code = 'UNAM' AND NOT EXISTS (SELECT 1 FROM public.formations WHERE code = 'PHYS-L3');
...
INSERT INTO public.formations (code, name, description, university_id, credits, duration_years, capacity, is_active) 
SELECT 'GEST-L3', 'Licence en Gestion', 'Formation en management et gestion', u.id, 180, 3, 120, true
FROM public.universities u
...
-- Insert sample scholarships with correct enum values
...
SELECT 'Bourse Excellence MESRS', 'excellence', 150000, 12, 'Moyenne générale > 16/20', '2024-12-31', ay.id, 50, 'Bourse pour les étudiants méritants', true
FROM public.academic_years ay
WHERE ay.is_current = true AND NOT EXISTS (SELECT 1 FROM public.scholarships WHERE name = 'Bourse Excellence MESRS');

INSERT INTO public.scholarships (name, type, amount, duration_months, criteria, deadline, academic_year_id, max_recipients, description, is_active) 
SELECT 'Bourse Sociale', 'social', 100000, 12, 'Revenu familial < 100.000 MRU/mois', '2024-11-30', ay.id, 200, 'Aide financière pour étudiants défavorisés', true
FROM public.academic_years ay
